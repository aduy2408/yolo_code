...
def zoom_group(boxes: list[tuple[float, float, float, float]]) -> tuple[int, ...]:
    """Choose one GT box, or a pair only when their centers are very close."""
    ...
    for index, box in enumerate(boxes):
        ...
        for other in range(index + 1, len(boxes)):
            ...
            distance = ((centers[index][0] - centers[other][0]) ** 2 + (centers[index][1] - centers[other][1]) ** 2) ** 0.5
            scale = max(box[2] - box[0], box[3] - box[1], other_box[2] - other_box[0], other_box[3] - other_box[1])
            if distance / max(scale, 1.0) <= 2.0:
                pair_area = (box[2] - box[0]) * (box[3] - box[1]) + (other_box[2] - other_box[0]) * (other_box[3] - other_box[1])
                candidates.append(((2, -distance / max(scale, 1.0), pair_area), (index, other)))
...
def zoom_bounds(boxes: list[tuple[float, float, float, float]], width: int, height: int) -> tuple[int, int, int, int]:
    ...
    selected = [boxes[index] for index in zoom_group(boxes)]
