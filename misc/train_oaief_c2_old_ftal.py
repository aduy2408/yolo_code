...
def run(a):
 ...
 p=a.project/"C2_OLD_FTAL"; p.mkdir(parents=True,exist_ok=True); sha=subprocess.check_output(["git","rev-parse","HEAD"],cwd=ROOT,text=True).strip()
 (p/"manifest.json").write_text(json.dumps({"experiment":"C2_OLD_FTAL","hypothesis":"old exploratory reconstruction-hard C2 with canonical FTAL","config":str(CONFIG),"commit_sha":sha,"seed":a.seed,"split":["val","test"],"nms_iou":0.5,"epochs":a.epochs,"patience":a.patience,"ftal":FTAL,"hf_repo_id":a.hf_repo_id},indent=2)+"\n")
 if not has(p,REQUIRED):
  seed(a.seed); model=YOLO(str(CONFIG)); model.load("yolov8n.pt",smart_transfer=True); model.train(data=str(data),epochs=a.epochs,patience=a.patience,imgsz=a.imgsz,batch=a.batch_size,device=a.device,workers=a.workers,amp=a.amp,seed=a.seed,deterministic=True,project=str(a.project),name="C2_OLD_FTAL",exist_ok=True,**FTAL)
 ...
 upload(p,a.hf_repo_id,a.remote_prefix.rstrip("/")+"/C2_OLD_FTAL"); print("COMPLETE C2_OLD_FTAL",flush=True)
def args():
