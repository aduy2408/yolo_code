#!/usr/bin/env python3
"""Train/evaluate GAP+FTAL classifier-capacity Detect head variants."""
...
workflow.EXPERIMENT = "levir_yolov8n_p2_gap_ftal_cls_capacity"
workflow.HF_REPO = "duyle2408/levir-yolov8n-p2-gap-ftal-cls-capacity-seed42"
workflow.VARIANTS = {
    "gap_ftal_reg64_cls64_dw": CONFIG_ROOT / "yolov8n_p2_fpn_only_gap_cls64_dw.yaml",
    "gap_ftal_reg64_cls64_fullconv": CONFIG_ROOT / "yolov8n_p2_fpn_only_gap_cls64_fullconv.yaml",
}
EXPECTED = {
    "gap_ftal_reg64_cls64_dw": (64, False),
    "gap_ftal_reg64_cls64_fullconv": (64, True),
}
...
def model_for(variant: str, pretrained: str):
    ...
    if head.head_share_mode != "none" or head.cls_head_width != width or head.cls_head_dense is not dense:
        raise ValueError(f"{variant}: wrong cls capacity settings")
    if head.cv2[0][0].conv.out_channels != 64:
        raise ValueError(f"{variant}: expected regression tower width 64")
    if dense:
        if head.cv3[0][0].conv.out_channels != 64 or head.cv3[0][1].conv.out_channels != 64:
            raise ValueError(f"{variant}: expected dense cls Conv3x3 width 64")
    elif not isinstance(head.cv3[0][0][0], DWConv) or head.cv3[0][0][1].conv.out_channels != 64:
        raise ValueError(f"{variant}: expected DW+PW cls tower width 64")
    return model
