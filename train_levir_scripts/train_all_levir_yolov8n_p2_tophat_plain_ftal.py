#!/usr/bin/env python3
"""Run one LEVIR YOLOv8n-P2 experiment combining top-hat cues with plain FTAL."""
...
from train_levir_scripts import train_all_levir_yolov8n_p2_plain_ftal_ggcf as plain_runner


CONFIG_ROOT = ROOT.parent / "models_related/models_config/yolov8/levir"
PLAIN_CONFIG = CONFIG_ROOT / "yolov8n_p2_fpn_only_plain_ggcf.yaml"
COMBINED_CONFIG = CONFIG_ROOT / "generated_input_cues/yolov8n_p2_tophat_plain_ftal.yaml"

COMBINED_CONFIG.parent.mkdir(parents=True, exist_ok=True)
_config = yaml.safe_load(PLAIN_CONFIG.read_text(encoding="utf-8"))
...
workflow = plain_runner.workflow
workflow.EXPERIMENT = "levir_yolov8n_p2_tophat_plain_ftal"
workflow.HF_REPO = "duyle2408/levir-yolov8n-p2-tophat-plain-ftal"
workflow.VARIANTS = {"top_hat_plain_ftal": COMBINED_CONFIG}


def train_kwargs(args: argparse.Namespace, data_yaml: Path, seed: int, amp: bool) -> dict[str, object]:
    kwargs = plain_runner.train_kwargs(args, data_yaml, seed, amp)
...
def model_for(variant: str, pretrained: str):
    ...
    if not isinstance(head, Detect) or head.f != [18]:
        raise ValueError("Expected plain P2 -> Detect([18])")
    if head.stride.tolist() != [4.0] or head.nl != 1 or not head.ggcf_refine:
        raise ValueError(f"Expected P2-only FTAL Detect stride [4], got {head.stride.tolist()}")
...
def parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    ...
    parser.set_defaults(runner=Path(__file__).resolve(), variants=["top_hat_plain_ftal"])
...
def main() -> None:
    ...
    workflow.Uploader = plain_runner.Uploader
    ...
    amp = {"top_hat_plain_ftal": True}
    if not args.no_smoke:
        args._variant = "top_hat_plain_ftal"
        amp["top_hat_plain_ftal"] = workflow.smoke("top_hat_plain_ftal", data_yaml, args)
    if args.smoke_only:
        return
    for seed in args.seeds:
        args._variant = "top_hat_plain_ftal"
        run_dir = workflow.train("top_hat_plain_ftal", seed, data_yaml, amp["top_hat_plain_ftal"], args)
        workflow.evaluate(run_dir, data_yaml, args)
        workflow.write_summaries(args)
        if uploader:
            uploader.upload_run(run_dir, "top_hat_plain_ftal", seed)
