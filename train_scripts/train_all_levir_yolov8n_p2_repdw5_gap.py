#!/usr/bin/env python3
"""Train, evaluate, and verify-upload P2 ResidualDWConv5 + GAP."""
...
VARIANTS = {
    ...
    "repdw5_gap": CONFIG_ROOT / "yolov8n_p2_fpn_only_repdw5_gap.yaml",
    "repdw3_gap": CONFIG_ROOT / "yolov8n_p2_fpn_only_repdw3_gap.yaml",
    "gap_repdw5": CONFIG_ROOT / "yolov8n_p2_fpn_only_gap_repdw5.yaml",
    "partial_repdw5_gap": CONFIG_ROOT / "yolov8n_p2_fpn_only_partial_repdw5_gap.yaml",
}
DW_SPECS = {
    "repdw5_gap": {"dw_layer": 19, "attention_layer": 20, "k": 5, "partial_ratio": 1.0},
    "repdw3_gap": {"dw_layer": 19, "attention_layer": 20, "k": 3, "partial_ratio": 1.0},
    "gap_repdw5": {"dw_layer": 20, "attention_layer": 19, "k": 5, "partial_ratio": 1.0},
    "partial_repdw5_gap": {"dw_layer": 19, "attention_layer": 20, "k": 5, "partial_ratio": 0.5},
}
REQUIRED = (
    "weights/best.pt",
    "weights/last.pt",
...
def prepare_split(args: argparse.Namespace) -> Path:
    from train_scripts.train_levir_scripts import train_all_levir_yolov8n_p2_routing as workflow
...
def model_for(variant: str, pretrained: str):
    ...
    from ultralytics.nn.modules import ChannelAttention, ResidualDWConv
    ...
    head = layers[-1]
    if variant == "gap":
        if not isinstance(layers[19], ChannelAttention) or head.f != [19]:
            raise TypeError("gap: expected ChannelAttention -> Detect")
    else:
        spec = DW_SPECS[variant]
        dw = layers[spec["dw_layer"]]
        attention = layers[spec["attention_layer"]]
        if not isinstance(dw, ResidualDWConv):
            raise TypeError(f"{variant}: ResidualDWConv did not resolve at layer {spec['dw_layer']}")
        if not isinstance(attention, ChannelAttention) or attention.descriptor != "avg" or head.f != [20]:
            raise TypeError(f"{variant}: expected configured ResidualDWConv/ChannelAttention -> Detect([20])")
        if dw.k != spec["k"] or dw.partial_ratio != spec["partial_ratio"]:
            raise ValueError(f"{variant}: wrong DW config k={dw.k}, partial_ratio={dw.partial_ratio}")
        if abs(float(dw.alpha.detach()) - 0.1) > 1e-6:
            raise ValueError(f"{variant}: alpha init is not 0.1")
...
def write_metadata(variant: str, run_dir: Path, seed: int, args: argparse.Namespace) -> None:
    ...
    manifest = {
        ...
        "topology": {
            "gap": "P2 -> ChannelAttention(avg) -> Detect([P2])",
            "repdw5_gap": "P2 -> ResidualDWConv(k=5, alpha=0.1) -> ChannelAttention(avg) -> Detect([P2])",
            "repdw3_gap": "P2 -> ResidualDWConv(k=3, alpha=0.1) -> ChannelAttention(avg) -> Detect([P2])",
            "gap_repdw5": "P2 -> ChannelAttention(avg) -> ResidualDWConv(k=5, alpha=0.1) -> Detect([P2])",
            "partial_repdw5_gap": "P2 -> ResidualDWConv(k=5, alpha=0.1, partial_ratio=0.5) -> ChannelAttention(avg) -> Detect([P2])",
        }[variant],
        "dw_layer": DW_SPECS.get(variant, {}).get("dw_layer"),
        "dw_kernel": DW_SPECS.get(variant, {}).get("k"),
        "dw_partial_ratio": DW_SPECS.get(variant, {}).get("partial_ratio"),
        "attention_layer": DW_SPECS.get(variant, {}).get("attention_layer", 19),
        "detect_from": head.f,
...
def parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    ...
    parser.add_argument("--project", type=Path, default=ROOT / "runs/levir_yolov8n_p2_dw_gap_variants")
    ...
    parser.add_argument("--hf-repo-id", default="duyle2408/levir-yolov8n-p2-dw-gap-variants")
    parser.add_argument("--seeds", type=int, nargs="+", default=[42])
    parser.add_argument("--variants", nargs="+", choices=list(VARIANTS), default=["repdw3_gap", "gap_repdw5", "partial_repdw5_gap"])
    parser.add_argument("--smoke-build", action="store_true", help="Build configured models and exit before upload/train.")
...
def main() -> None:
    ...
    for variant in args.variants:
        model_for(variant, args.pretrained if not args.smoke_build else "")
    if args.smoke_build:
        return
