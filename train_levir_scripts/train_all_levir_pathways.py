#!/usr/bin/env python3
"""Train/evaluate plain P2 + dense classification pathways on LEVIR dataset."""
...
workflow.EXPERIMENT = "levir_yolov8n_p2_gap_ftal_pathways"
workflow.HF_REPO = "duyle2408/levir-yolov8n-p2-gap-ftal-pathways-seed42"
workflow.VARIANTS = {
    "plain": CONFIG_ROOT / "yolov8n_p2_fpn_only_plain.yaml",
    "structural_energy": CONFIG_ROOT / "yolov8n_p2_fpn_only_plain.yaml",
    "feature_polarity": CONFIG_ROOT / "yolov8n_p2_fpn_only_plain.yaml",
    "global_reference": CONFIG_ROOT / "yolov8n_p2_fpn_only_plain.yaml",
}
...
def model_for(variant: str, pretrained: str):
    ...
    # Dynamically read and write a temporary config YAML file with pathway_mode injected
    plain_yaml_path = CONFIG_ROOT / "yolov8n_p2_fpn_only_plain.yaml"
    with open(plain_yaml_path, "r", encoding="utf-8") as f:
        cfg_dict = yaml.safe_load(f)
    
    cfg_dict["pathway_mode"] = variant if variant != "plain" else "none"
    cfg_dict["pathway_alpha"] = 0.0
...
def parse_args() -> argparse.Namespace:
    ...
    parser.set_defaults(current_variant="plain", runner=Path(__file__).resolve())
