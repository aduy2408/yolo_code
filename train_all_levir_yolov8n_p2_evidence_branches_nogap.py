#!/usr/bin/env python3
"""Train matched LEVIR P2 evidence ablations without GAP ChannelAttention."""
...
VARIANTS = {
    "extra8_joint_nogap": (CONFIG_ROOT / "yolov8n_p2_extra8_joint_nogap.yaml", {}),
    "isolated_evidence8_nogap": (CONFIG_ROOT / "yolov8n_p2_gradient_isolated_evidence_nogap.yaml", {"evidence_aux_gain": 1.0}),
    "scale_disappearance8_nogap": (CONFIG_ROOT / "yolov8n_p2_scale_disappearance_evidence_nogap.yaml", {}),
    "resolution_extra8_nogap": (CONFIG_ROOT / "yolov8n_p2_extra8_joint_nogap.yaml", {"resolution_enabled": True, "mosaic_postprocess_enabled": True}),
    "resolution_conditioned8_nogap": (CONFIG_ROOT / "yolov8n_p2_resolution_conditioned_evidence_nogap.yaml", {"resolution_enabled": True, "mosaic_postprocess_enabled": True, "aug_state_gain": 0.1}),
}
...
def preflight_variant(variant: str) -> None:
    ...
    expected_detect_from = [20] if expected_module == "ScaleDisappearanceEvidence" else [19]
    if "ChannelAttention" in modules:
        raise ValueError(f"{variant}: no-GAP config unexpectedly contains ChannelAttention")
...
def write_metadata(variant: str, run_dir: Path, seed: int, args: argparse.Namespace) -> None:
    ...
    manifest = {
        ...
        "variant_settings": VARIANTS[variant][1], "gap_channel_attention": False,
    }
...
def parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    ...
    parser.add_argument("--project", type=Path, default=ROOT / "runs/levir_yolov8n_p2_evidence_branches_nogap")
