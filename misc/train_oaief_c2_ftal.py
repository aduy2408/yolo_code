#!/usr/bin/env python3
"""Train, evaluate, and upload the corrected C2 hard-negative variant with FTAL."""
...
CONFIG=ROOT/"models_related/models_config/yolov8/levir/yolov8n_p2_levir_complement_hard_negative.yaml"
...
def run(a):
 ...
 p=a.project/"C2_FTAL"; p.mkdir(parents=True,exist_ok=True); sha=subprocess.check_output(["git","rev-parse","HEAD"],cwd=ROOT,text=True).strip()
 (p/"manifest.json").write_text(json.dumps({"experiment":"C2_FTAL","hypothesis":"detector-confidence hard-negative C2 with canonical FTAL","config":str(CONFIG),"commit_sha":sha,"seed":a.seed,"split":["val","test"],"nms_iou":0.5,"epochs":a.epochs,"patience":a.patience,"ftal":FTAL,"hf_repo_id":a.hf_repo_id},indent=2)+"\n")
 if not has(p,REQUIRED):
  seed(a.seed); model=YOLO(str(CONFIG)); model.load("yolov8n.pt",smart_transfer=True); model.train(data=str(data),epochs=a.epochs,patience=a.patience,imgsz=a.imgsz,batch=a.batch_size,device=a.device,workers=a.workers,amp=a.amp,seed=a.seed,deterministic=True,project=str(a.project),name="C2_FTAL",exist_ok=True,**FTAL)
 if not has(p,REQUIRED): raise FileNotFoundError(p)
 if not (p/"evaluation_metrics.json").is_file(): evaluate(p,data,a)
 if not has(p,COMPLETE): raise FileNotFoundError(p)
 upload(p,a.hf_repo_id,a.remote_prefix.rstrip("/")+"/C2_FTAL"); print("COMPLETE C2_FTAL",flush=True)
def args():
 p=argparse.ArgumentParser(); p.add_argument("--data-root",type=Path,required=True); p.add_argument("--dataset-root",type=Path,required=True); p.add_argument("--project",type=Path,required=True); p.add_argument("--remote-prefix",required=True); p.add_argument("--hf-repo-id",required=True); p.add_argument("--seed",type=int,default=42); p.add_argument("--epochs",type=int,default=100); p.add_argument("--patience",type=int,default=0); p.add_argument("--imgsz",type=int,default=512); p.add_argument("--batch-size",type=int,default=8); p.add_argument("--device",default="0"); p.add_argument("--workers",type=int,default=4); p.add_argument("--amp",action=argparse.BooleanOptionalAction,default=True); return p.parse_args()
