"""Train, test, and upload the YOLOv8 edge RepC2f Varroa configs."""
...
TEST_PROJECT = ROOT / "runs/detect/yolo_related/runs/test_edge"
...
def find_best_weights(run_names: list[str]) -> list[Path]:
    """Find best.pt files for the requested edge runs."""
    best_paths = []
...
def run_test_inference(run_names: list[str], data_test: str) -> Path | None:
    """Evaluate trained edge checkpoints on the test split and save a summary CSV."""
    ...
    summary_path = TEST_PROJECT / "test_summary_edge.csv"
...
def upload_runs_to_hf(hf_token: str | None = None, repo_id: str | None = None) -> None:
    """Upload edge train/test outputs to Hugging Face when HF_TOKEN is available."""
    ...
    repo_id = repo_id or os.environ.get("HF_REPO_ID", f"duyle2408/varroa-yolo-edge-yolov8{MODEL_SCALE}")
...
run_names = []
print(f"[*] ĐANG CHẠY EDGE YOLOv8{MODEL_SCALE}: {len(EDGE_CONFIGS)} configs")
