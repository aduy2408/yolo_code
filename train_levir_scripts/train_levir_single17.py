#!/usr/bin/env python3
"""Train/evaluate LEVIR P2 single17 and control architectures."""
...
EXPERIMENT = "levir_yolov8n_p2_single17"
HF_REPO = "duyle2408/levir-yolov8n-p2-single17"
PLAIN_YAML = ROOT.parent / "models_related/models_config/yolov8/levir/yolov8n_p2_fpn_only_plain.yaml"
VARIANTS = {
    "single17": CONFIG_ROOT / "yolov8n_p2_single17.yaml",
    "single17_no_form": CONFIG_ROOT / "yolov8n_p2_single17_no_form.yaml",
    "single_raw_form": CONFIG_ROOT / "yolov8n_p2_single_raw_form.yaml",
}
...
def _copy_semantic_plain_initialization(target, plain) -> int:
    """Copy downstream layer parameters from plain YOLO P2 initialization."""
    src = plain.model.model
    dst = target.model.model
    copied = 0

    # Target layer 0 has no matching backbone weight transfer because it's a completely new single stem.
