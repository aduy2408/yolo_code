#!/usr/bin/env python3
"""Train/evaluate the YOLOv8 P2 Plain + CRC (no FTAL) model."""
...
workflow.EXPERIMENT = "levir_yolov8n_p2_plain_crc"
workflow.HF_REPO = "duyle2408/levir-yolov8n-p2-plain-crc"
workflow.VARIANTS = {
    "plain_crc": CONFIG_ROOT / "yolov8n_p2_fpn_only_plain_crc.yaml",
}
...
def train_kwargs(args: argparse.Namespace, data_yaml: Path, seed: int, amp: bool) -> dict[str, object]:
    # Do not set factorized_tal_target=True (disables FTAL)
...
def model_for(variant: str, pretrained: str):
    ...
    from ultralytics.nn.modules import Detect, RingContextCls
