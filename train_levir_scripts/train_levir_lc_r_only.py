#!/usr/bin/env python3
"""Train/evaluate LEVIR P2 local-contrast-basis with LC_R_only simplified backbone."""
...
EXPERIMENT = "levir_yolov8n_p2_lc_r_only"
HF_REPO = "duyle2408/levir-yolov8n-p2-lc-r-only"
PLAIN_YAML = ROOT.parent / "models_related/models_config/yolov8/levir/yolov8n_p2_fpn_only_plain.yaml"
VARIANTS = {
    "lc_r_only": CONFIG_ROOT / "yolov8n_p2_lc_r_only.yaml",
}
...
def _copy_semantic_plain_initialization(target, plain) -> int:
    ...
    # Target layer 0 has no active main backbone weight transfers since we run R-only,
    # but we transfer main components if we want to run main control paths.
    # For R-only simplified, we only copy the downstream layers.
    copied = 0
